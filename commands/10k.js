...
function CreateManyCards(message, args) {
	...
	for (q = 0; q < 100; q++) {
	exec(`bash ~/mtg-rnn/10k.sh ${args[1]} ${args[5] + q} ${args[2]} "|0" ${args[6]} mtg`, (err, stdout, stderr) => {
		if (err) {
...
function Splitcards(message, args) {	
	fs.readFile('/mnt/c/mtg-rnn/primepretty.txt', 'utf8' , function (err, data) {
		if (err) {
			...
			//	content: `${message.author}, you requested a text file, or more than 50 cards (${args[3]}), so you get them in raw text!`,
