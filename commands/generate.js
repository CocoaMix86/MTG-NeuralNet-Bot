...
function ArgsCheck(message, args) {
	...
	Generating the batch of cards now... Please wait a moment!`).then(msg => {
		...
		else if (args[9] == 'multicolor') {
			fs.readFile('/mnt/c/Discord Bot/mtgnet/format-multicolor.txt', 'utf8' , function (err, data) {
				if (err) { }
				else {
					seeds = data.split('\r\n')
					CreateSeededCard(message, args, seeds)
		...
		else if (args[9] == 'uniquecosts') {
			fs.readFile('/mnt/c/Discord Bot/mtgnet/format-uniquecosts.txt', 'utf8' , function (err, data) {
				if (err) { console.log("uniquecosts failed")}
				else {
					seeds = data.split('\r\n')
					CreateSeededCard(message, args, seeds)
				}
