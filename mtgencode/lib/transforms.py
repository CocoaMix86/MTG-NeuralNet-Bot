...
def text_unpass_1_choice(s, delimit = False):
    ...
    for choice in sorted(choices, lambda x,y: cmp(len(x), len(y)), reverse = True):
        fragments = choice[1:-1].split(bullet_marker) #splits on `=`, and cuts off [ ] on the ends
        countfrag = fragments[0] #get number of options, stored in 0
        optfrags = fragments[1:] #get number of options, all elements in array after 0
        choicecount = int(utils.from_unary(re.findall(utils.number_unary_regex, countfrag)[0])) #convert countfrag to int
        newchoice = ''

        if choicecount == 0:
            if len(optfrags) == 2: #countfrag
...
def text_unpass_8_unicode(s):
    s = s.replace("non~", "non-") #don't want emdash after non
    s = s.replace("ii ~", "II ~").replace("iii ~", "III ~") #uppercase chapter abilities
    #uppercase some edge cases
    s = s.replace(" i ", " I ") #catches stand-alone 'i' (usually in flavor text)
    #do unicode things
    s = s.replace(" " + dash_marker, " " + u'\u2014')
    s = s.replace(dash_marker, "-")
    s = s.replace(bullet_marker, u'\u2022')
    #fix up dice rolling markers now
    s = s.replace(dice_marker, '|')
    s = s.replace("**", "-")
    return s  